...
import {
    ...
    Pencil, Search, CalendarDays, Zap,
} from 'lucide-react';
            ...
            cell: ({ row }) => {
                ...
                return (
                    <span className={cn(
                        ...
                        type === 'ANNUAL_PLAN' ? 'bg-primary/10 text-primary' : 'bg-orange-500/10 text-orange-600 dark:text-orange-400',
                    )}>
                        {type === 'ANNUAL_PLAN' ? <CalendarDays className="h-3 w-3" /> : <Zap className="h-3 w-3" />}
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="text-right">
                    <span className="font-semibold text-sm tabular-nums">{row.original.daysWorked}</span>
...
export function WorkLogDataTable({ logs, onUpdate }: WorkLogDataTableProps) {
    ...
    const totalFiltered = table.getFilteredRowModel().rows.reduce(
        (sum, row) => sum + row.original.daysWorked, 0,
    );
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="flex items-center gap-3 flex-wrap">
                ...
                <Select value={typeFilter} onValueChange={(v) => table.getColumn('type')?.setFilterValue(v)}>
                ...
                <p className="ml-auto text-xs text-muted-foreground">
                    <span className="font-medium text-foreground">{table.getFilteredRowModel().rows.length}</span> kayıt · Toplam <span className="font-semibold text-foreground">{totalFiltered} gün</span>
                </p>
            </div>
                ...
                <Table>
                    ...
                    <TableBody>
                        {table.getRowModel().rows.length > 0 ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={columns.length} className="h-24 text-center text-sm text-muted-foreground/60">
