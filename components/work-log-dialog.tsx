		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className='sm:max-w-[425px]'>
				<DialogHeader>
					...
					<DialogDescription>
						<span className='font-semibold text-foreground'>{title}</span> için çalışma
						sürenizi girin.
					</DialogDescription>
				</DialogHeader>
				<form onSubmit={handleSubmit}>
								...
								{isRange ? (
									<>
										...
										<p className='text-[11px] text-muted-foreground/70'>
											{businessDays
												.map((d) => format(d, 'd MMM', { locale: tr }))
												.join(', ')}
										</p>
									</>
								) : (
									<p className='text-xs text-muted-foreground'>
										<span className='font-semibold text-foreground'>
											{format(businessDays[0], 'd MMMM yyyy', { locale: tr })}
										</span>{' '}
										için{' '}
					...
					<DialogFooter>
						<Button
							type='submit'
							disabled={isLoading || businessDays.length === 0}
							size='sm'
						>
